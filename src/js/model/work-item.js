 ...
 ***************************************************************************/
try {
    module.exports = exports = WorkItem;
} catch (e) {}
...
WorkItem.elapsedTime = function(compareTime, standardTime){
    standardTime = (standardTime) ? standardTime : new Date().getTime();
    return standardTime - compareTime;
};
WorkItem.checkElapsedTime = function(compareTime, limitTime, standardTime){
    return limitTime < WorkItem.elapsedTime(compareTime, standardTime);
};
...
WorkItem.makeArgumentListByComma = function(){
...
WorkItem.runMethodByRange = function(range, targetDate, lastStartTime){
    ...
    if (startBraceIndex != -1){
        ...
        args = range.substring(startBraceIndex +1, endBraceIndex);
    }else{
        methodName = defaultMethodName;
        args = trimedRange;
    }
    //Run
    return WorkItem.runMethod(methodName, {date:targetDate, lastStartTime:lastStartTime}, WorkItem.makeArgumentListByComma(args));
};
